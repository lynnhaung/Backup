...
*/
		...
		function nav_link($navlink, $class)
		{
			if (isset($navlink['url']))
				$this->output(
					...
					'<div class="qa-'.$class.'-img" title="'.$navlink['label'].'"></div>'.
					'<span>'.$navlink['label'].'</span></a>'
				);
	...
	*/
		...
		function ranking($ranking)
		{
            ...
            for ($i = 0; $i < count($ranking['items']); $i++) {
                echo '<div>';
                $this->ranking_item(@$ranking['items'][$i], $class, null);
                echo '</div>';
            }
		...
		{
			echo '<td class="'.$class.'-spacer">&nbsp;</td>';
		}
		
		function ranking_count($item, $class)
		{
            echo '<span class="'.$class.'-count">'.$item['count'].' &#215;'.'</span>';
		}
		
		function ranking_label($item, $class)
		{
            echo '<span class="'.$class.'-label">'.$item['label'].'</span>';
		}
		
		function ranking_score($item, $class)
		{
            echo '<span class="'.$class.'-score">'.$item['score'].'</span>';
		}

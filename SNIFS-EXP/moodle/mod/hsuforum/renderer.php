...
class mod_hsuforum_renderer extends plugin_renderer_base {
    ...
    public function discussion_sorting(hsuforum_lib_discussion_sort $sort) {
		...
		$output = $this->append_group_script($output);
		
        return html_writer::tag('div', $output, array('class' => 'hsuforum_discussion_sort'));
    }
	
	public function append_group_script($output) {
		
		if (isset($_SESSION["discuss_group_name"])) {
			$output .= '<script src="js/jquery-3.3.1.min.js"></script>';
			$output .= '<script src="js/script.js"></script>';
			$output .= '<input type="hidden" id="discuss_group_name" value="' . $_SESSION["discuss_group_name"] . '" />';
		}
    ...
    public function simple_edit_post($cm, $isedit = false, $postid = 0, array $data = array()) {
		...
		$legend = $legend . '<a name="hsuforum-reply-anchor" id="hsuforum_reply_anchor"></a>
	<a class="float reply button" style="position:fixed; bottom: 1rem; right: 1rem; background-color: #F44336;color:white;width: 56px;height: 56px;padding: 0;cursor: pointer;overflow: hidden;user-select: none;z-index: 1;color: #fff;border-radius: 50%;text-align: center;letter-spacing: .5px;text-decoration: none;line-height: 56px;font-size: 50px;" onclick="document.getElementsByClassName(\'form-checker-added\')[0].scrollIntoView({behavior: \'smooth\'});">+</a>';
